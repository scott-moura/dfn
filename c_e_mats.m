...
function [A,B,C] = c_e_mats(p,c_ex)
...
% BC2
N1(2,p.Nxn-1) = De_ns/(p.L_n * p.delta_x_n);
N1(2,p.Nxn) = De_ns/(p.L_s * p.delta_x_s);
N2(2,2) = -De_ns/(p.L_n * p.delta_x_n) - De_ns/(p.L_s * p.delta_x_s);

% BC3
N1(3,p.Nxn+p.Nxs-2) = De_sp/(p.L_s * p.delta_x_s);
N1(3,p.Nxn+p.Nxs-1) = De_sp/(p.L_p * p.delta_x_p);
N2(3,3) = -De_sp/(p.L_s * p.delta_x_s) - De_sp/(p.L_p * p.delta_x_p);
