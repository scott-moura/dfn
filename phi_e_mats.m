...
function [F1,F2,F3,varargout] = phi_e_mats(p,c_ex)
...
beta_n = -(2*p.R*p.T_amp)/(p.alph*p.Faraday) * (1 - p.t_plus) * (1 + 0) / (2 * p.L_n * p.delta_x_n);
beta_s = -(2*p.R*p.T_amp)/(p.alph*p.Faraday) * (1 - p.t_plus) * (1 + 0) / (2 * p.L_s * p.delta_x_s);
beta_p = -(2*p.R*p.T_amp)/(p.alph*p.Faraday) * (1 - p.t_plus) * (1 + 0) / (2 * p.L_p * p.delta_x_p);

gam = -(2*p.R*p.T_amp)/(p.alph*p.Faraday) * (1 - p.t_plus) * (1 + 0);
