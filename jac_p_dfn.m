...
JF(ind_ce(1:Nn),15) = 0; %-(1/p.epsilon_e_n) * JF_ce_ie(1:Nn);
% wrt epsilon_e_p
% JF(ind_ce(end-Np+1:end),17) = -beta_p*(1-p.t_plus)/(p.epsilon_e_p^2*p.Faraday) * Grad_p * i_ep;
JF(ind_ce(end-Np+1:end),17) = 0; %-(1/p.epsilon_e_p) * JF_ce_ie(end-Np+1:end);
...
%                     * Grad_p * log(c_ep);
JG(ind_phi_e,6) = 0; %1/(1-p.t_plus) * F3_pe*log(c_ex);
...
%                     * Grad_p * log(c_ep);
JG(ind_phi_e,10) = 0; %1/(1+0) * F3_pe*log(c_ex);
